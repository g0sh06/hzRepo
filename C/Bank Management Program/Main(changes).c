     ...
     if(!strcmp(testpassword, c.password)){
       while(cont == 'y'){
        ...
        fp = fopen(strcat(filename, ".dat"), "r");
          ...
          fread(&c1, sizeof(struct user), 1, fp);
        }
        fclose(fp);
        if(amount > c.balance){printf("\n Insufficient balance");}
        else
        {
          fp = fopen(filename, "w");
          c1.balance += amount;
          fwrite(&c1, sizeof(struct user), 1, fp);

          fclose(fp);
          if(fwrite != NULL){
            printf("\nYou have succesfully transfered the money to your bank account");
            strcpy(filename, c.phone);
            fp = fopen(strcat(filename, ".dat"), "w");
            c.balance -= amount;
            fwrite(&c1, sizeof(struct user), 1, fp);
